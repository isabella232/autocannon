...
function _run (opts, cb, tracker) {
  ...
  const urls = ofURL(opts.url, true).map(url => {
    ...
    url.expectResponse = opts.expectResponse
  ...
  function initialiseClients (clients) {
    for (let i = 0; i < opts.connections; i++) {
      ...
      client.on('responseMismatch', onExpectResponseMismatch)
    ...
    function onExpectMismatch (bodyStr) {
      for (let i = 0; i < opts.pipelining; i++) {
        tracker.emit('reqMismatch', bodyStr)
      }
    ...
    function onExpectResponseMismatch ({ status, body }) {
      for (let i = 0; i < opts.pipelining; i++) {
        tracker.emit('reqResponseMismatch', { status, body })
      }
  ...
  function checkOptsForErrors () {
    ...
    if (opts.expectBody && opts.requests !== DefaultOptions.requests) {
      errorCb(
        new Error('expectBody cannot be used in conjunction with requests')
      )
      return true
    }

    if (opts.expectResponse && opts.requests !== DefaultOptions.requests) {
      errorCb(
        new Error('expectResponse cannot be used in conjunction with requests')
      )
      return true
    }
